...
function parse(data) {
  ...
  _.each(lines, function(lineContent, i) {
    ...
    if (s(lineContent).trim().startsWith('BEGIN:MSG')) {
      message = {};
      message.status = 'unread';
      message.text = lineContent[i + 1];
    }

    if (s(lineContent).trim().startsWith('END:MSG')) {
    ...
    _.each(attributes, function(attribute) {
      ...
      if (s(lineContent).contains(attribute)) {
        if (attribute === 'STATUS:') {
          message.status = s(line.slice(7)).clean().value();
        } else {
